...
def calc_loss_test(pred1, pred2, target, metrics):
...
def build_trt_engine(onnx_file_path):
...
def main(args):
    ...
    if use_tensorrt:
        onnx_file_path = cfg.get('onnx_file_path', 'model.onnx')
        trt_engine = build_trt_engine(onnx_file_path)
        input_data = np.random.random((1, 3, 224, 224)).astype(np.float32)
        output = infer_with_trt(trt_engine, input_data)
        print("TensorRT Inference output:", output)
    else:
    ...
    if data_cfg['name'] == 'radio':
        dataset = loaders.RadioUNet_c(phase="test", dir_dataset=data_cfg['img_folder'])
    else:
        raise NotImplementedError
    ...
    def sample(self):
        ...
        with torch.no_grad():
            ...
            if self.cfg.sampler.sample_type == 'slide':
                ...
                print(f'Average whole_sample time: {avg_time:.4f} seconds')
